...
export class UserService {
    ...
    addUser(id, name, surname, dateOfBirth, phone, email, timeStamp) {
        ...
        const newUser = new User(id, name, surname, dateOfBirth, phone, email, timeStamp);
    ...
    editUser(userId: string, name: string, surname: number, dateOfBirth: string, phone: string, email: string, timeStamp: string) {
    ...
    findUser(userId: string): [User, number] {
        const userIndex = this.users.findIndex(activeUser => activeUser.id === userId);
